...
class DonchianAdxConfig(SBTBarStrategyConfig, kw_only=True, frozen=True):
...
class DonchianAdx(SBTStrategy):
    def __init__(self, config: DonchianAdxConfig) -> None:
    ...
    def on_trading_bar(self, bar) -> None:
        ...
        cp = self.config.channel_period
        if len(self._highs) < cp + 1:
            return

        upper = max(self._highs[-cp - 1 : -1])
        lower = min(self._lows[-cp - 1 : -1])
        ...
        if not self.in_position:
            ...
            if self.position_side == OrderSide.BUY and close < lower:
                self.exit_market()
            elif self.position_side == OrderSide.SELL and close > upper:
                self.exit_market()
