...
1. New file `sbt/strategies/ohlc/<snake_name>.py` (bar-driven) or `sbt/strategies/l2/<snake_name>.py` (order-book-driven) containing:
   - Bar-driven: `<Name>Config(SBTBarStrategyConfig, kw_only=True, frozen=True)` imported from `...plugins`; order-book: `<Name>Config(SBTStrategyConfig, kw_only=True, frozen=True)`. The tier already carries the runner-injected fields (`instrument_id`, `bar_type` [bar tier only], `capital`, `leverage`, `backtest_start_date`, `active_from`) — declare only the optional `plugins: tuple[str, ...] = (...)` opt-in (e.g. `("vol_scaling",)`) plus paper parameters with sensible defaults. Unknown config keys are rejected loudly by the runner. `kw_only=True` is REQUIRED on every subclass — msgspec does not inherit it, and overriding an inherited field without it breaks struct construction.
